...
class Gate(object):
    ...
    def __init__(self, name, gatetype, fanin=(), fanout=(), level=-1):
    ...
    def evaluate(self, ckt):
        ...
        if self.gatetype == "UNDEFINED":
            self.value = Roth.X
        if self.gatetype == "MUX":
            # Better have exactly three fan-in
            if (len(statein) != 3):
                raise KeyError("Incorrect inputs for MUX gate")
            val_s = statein[0]
            val_1 = statein[1]
            val_0 = statein[2]
            # Perform MUX operations
            not_s = Roth.invert(statein[0])
            and_0 = Roth.operate("AND", (not_s, val_0))
            and_1 = Roth.operate("AND", (val_s, val_1))
            self.value = Roth.operate("OR", (and_0, and_1))
...
class Circuit(object):
    ...
    def reset_levels(self):
        """ Set levels back to init value for all gates in the circuit """
        for gate in self.gatemap.values():
            gate.level = -1
    ...
    def evaluate(self):
        ...
        if len(delayed) > 0:
            print("Delayed Set: ", [x.name for x in delayed])
            raise RuntimeError("Feedback loop present in circuit")

        # No path to primary output if any of the outputs are untouched
        for out in self.outputs:
            if not self.gatemap[out].flag:
                print(self.inputs)
                raise RuntimeError("No path to primary output {} ({})".format(out, self.gatemap[out].gatetype))
